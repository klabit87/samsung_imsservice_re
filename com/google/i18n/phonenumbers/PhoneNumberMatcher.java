...
final class PhoneNumberMatcher implements Iterator<PhoneNumberMatch> {
    ...
    public boolean checkNumberGroupingIsValid(Phonenumber.PhoneNumber number, CharSequence candidate, PhoneNumberUtil util, NumberGroupingChecker checker) {
        ...
        if (checker.checkGroups(util, number, normalizedCandidate, getNationalNumberGroups(util, number))) {
            return true;
        }
        ...
        if (alternateFormats != null) {
            for (Phonemetadata.NumberFormat alternateFormat : alternateFormats.numberFormats()) {
                if ((alternateFormat.leadingDigitsPatternSize() <= 0 || this.regexCache.getPatternForRegex(alternateFormat.getLeadingDigitsPattern(0)).matcher(nationalSignificantNumber).lookingAt()) && checker.checkGroups(util, number, normalizedCandidate, getNationalNumberGroups(util, number, alternateFormat))) {
                    return true;
                }
            }
        }
