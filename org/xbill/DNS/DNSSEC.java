...
public class DNSSEC {
    ...
    private static class ECKeyInfo {
        ...
        ECKeyInfo(int length2, String p_str, String a_str, String b_str, String gx_str, String gy_str, String n_str) {
            this.length = length2;
    ...
    private static PublicKey toECDSAPublicKey(KEYBase r, ECKeyInfo keyinfo) throws IOException, GeneralSecurityException, MalformedKeyException {
        DNSInput in = new DNSInput(r.getKey());
        return KeyFactory.getInstance(DiagnosisConstants.RCSM_MTYP_EC).generatePublic(new ECPublicKeySpec(new ECPoint(readBigInteger(in, keyinfo.length), readBigInteger(in, keyinfo.length)), keyinfo.spec));
    }
    ...
    public static RRSIGRecord sign(RRset rrset, DNSKEYRecord key, PrivateKey privkey, Date inception, Date expiration, String provider) throws DNSSECException {
        ...
        rrsig.setSignature(sign(privateKey, key.getPublicKey(), alg, digestRRset(rrsig, rrset), provider));
        return rrsig;
    }
    ...
    static SIGRecord signMessage(Message message, SIGRecord previous, KEYRecord key, PrivateKey privkey, Date inception, Date expiration) throws DNSSECException {
        ...
        sig.setSignature(sign(privateKey, key.getPublicKey(), alg, out.toByteArray(), (String) null));
        return sig;
    }
